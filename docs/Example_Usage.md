...
Please note, the following example uses AES "256" encryption from m17-tools, which is then decrypted by m17-fme using a 128-bit segment of the same key, noting that m17-tools is hard coded to only with in AES 128-bit mode, and thus, for now, m17-fme will do the same to maintain compatibility.
