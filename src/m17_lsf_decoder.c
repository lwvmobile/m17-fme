...
{
  int i;
  unsigned long long int lsf_dst = (unsigned long long int)ConvertBitIntoBytes(&m17d->lsf[0], 48);
  unsigned long long int lsf_src = (unsigned long long int)ConvertBitIntoBytes(&m17d->lsf[48], 48);
  uint16_t lsf_type = (uint16_t)ConvertBitIntoBytes(&m17d->lsf[96], 16);
  ...
  fprintf (stderr, "\n");
  decode_callsign_data(m17d, lsf_dst, lsf_src);
  ...
  uint32_t tsi = (uint32_t)ConvertBitIntoBytes(&m17d->lsf[112], 32); //OTA LSB 32-bit value
  ...
  for (i = 0; i < 14; i++)
    m17d->meta[i] = (uint8_t)ConvertBitIntoBytes(&m17d->lsf[(i*8)+112], 8);

  //Decode Meta Data
  if (lsf_et == 0 && m17d->meta[0] != 0) //not sure if this applies universally, or just to text data byte 0 for null data
  {
    uint8_t meta[15]; meta[0] = lsf_es + 90; //add identifier for pkt decoder
    for (i = 0; i < 14; i++) meta[i+1] = m17d->lsf[i];
    fprintf (stderr, "\n ");
    decode_pkt_contents (meta, 15); //decode META
  }
  ...
  if (lsf_et == 2)
  {
    fprintf (stderr, " IV: ");
    for (i = 0; i < 16; i++)
      fprintf (stderr, "%02X", m17d->meta[i]);
  }
