...
{
  if (super->opts.use_m17_rfa_decoder == 1) //differentiate input names so that the mixer will remember the input device used locally
    super->pa.pa_input_device = pa_simple_new(NULL, "M17-FME1", PA_STREAM_RECORD, NULL, "RF Input", &super->pa.input, NULL, &super->pa.inputlt, &err);
  else super->pa.pa_input_device = pa_simple_new(NULL, "M17-FME2", PA_STREAM_RECORD, NULL, "Voice Input", &super->pa.input, NULL, &super->pa.inputlt, &err);
...
{
  // super->pa.pa_output_device_rf = pa_simple_new(NULL, "M17-FME3", PA_STREAM_PLAYBACK, NULL, "RF Output", &super->pa.output_rf, NULL, &super->pa.outputlt, &err);
  super->pa.pa_output_device_rf = pa_simple_new(NULL, "M17-FME3", PA_STREAM_PLAYBACK, NULL, "RF Output", &super->pa.output_rf, NULL, NULL, &err);
...
{
  // super->pa.pa_output_device_vx = pa_simple_new(NULL, "M17-FME4", PA_STREAM_PLAYBACK, NULL, "Voice Output", &super->pa.output_vx, NULL, &super->pa.outputlt, &err);
  super->pa.pa_output_device_vx = pa_simple_new(NULL, "M17-FME4", PA_STREAM_PLAYBACK, NULL, "Voice Output", &super->pa.output_vx, NULL, NULL, &err);
