...
void encode_str(Super * super)
{
  ...
  while (!exitflag) //while the software is running
  {
    ...
    //needed for internal encoder to work right if/when switching modes
    if (super->opts.m17_str_encoder_dt == 3) st = 3; //this is set to 3 IF -S user text string is called at CLI
    else st = 2; //otherwise, just use 3200 voice
    ...
    //Version 3.0 spec items
    payload_contents = 0;
    encryption_type = 0;
    signature = 0;
    // meta_contents = 0; //don't reset this one?
    channel = can;
    ...
    if (super->m17e.str_encoder_tx == 1) //when toggled on
    {
      ...
      if (new_lsf == 1)
      {
        ...
        for (i = 0; i < 2; i++) //send LSF multiple times on STR data to ensure it syncs properly
    ...
    else //if not tx, reset values, drop sync
    {
      ...
      //Start Round Robin Baconator Function -- maybe use a one-time flag to set this?
      // if (fsn != 0 && lich_cnt == 0) //constantly runs, so disable debug print items in baconator function
      {
        meta_contents = meta_round_robin_v3_baconator (super, fsn, m17_lsf);
